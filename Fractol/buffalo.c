...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/03/16 18:33:33 by fshanaha          #+#    #+#             */
/*   Updated: 2019/03/19 16:51:47 by fshanaha         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "include/fractol.h"

static	void		buffalo_set_0(t_frl *f)
{
	f->reim.newre = 0;
	f->reim.newim = 0;
	f->reim.oldim = 0;
...
static	void		buffalo_alg2(t_frl *f)
{
	int		i;
	...
	while (f->y < f->y_max)
	{
												...
												< 4 && ++i < f->maxiterations)
		{
			...
			f->reim.newre = fabs(f->reim.oldre * f->reim.oldre - f->reim.oldim *
...
static	void		*buffalo_alg(void *fra)
{
	int		buff;
	t_frl	*f;
...
void				buffalo(t_frl *f)
{
	...
	while (++i < THREAD)
	{
		...
		pthread_create(&t[i], NULL, buffalo_alg, &fr[i]);
	}
	while (i--)
